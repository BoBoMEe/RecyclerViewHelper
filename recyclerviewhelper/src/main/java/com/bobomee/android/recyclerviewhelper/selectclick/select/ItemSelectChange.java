...
/**
 * Created on 2016/12/18.下午6:33.
 *
 * @author bobomee.
 */

public class ItemSelectChange
    extends ListenerImpl<ItemSelectChange.OnItemSelectChangeListener> {

  public static interface OnItemSelectChangeListener{
    void onItenSelectChange(StateManager.CheckedStates mCheckedStates);
  }

  public void selectChange(StateManager.CheckedStates mCheckedStates) {
    List<OnItemSelectChangeListener> from = from();
    if (null != from && !from.isEmpty()) {
      for (OnItemSelectChangeListener onItemSelectChangeListener : from) {
